...
/**
 * 黑白棋中级算法（最多可向后推4步，AI一般）
 * 
 * @author cy
 *
 */
public class IntermediateAlgorithm extends BaseAlgorithm {

	private final int max_steps = 2;

	public IntermediateAlgorithm(int id, int[][] last_status,
			MyPoint last_step, LinkedList<MyPoint> legalPoints, int count) {
	...
	public void setValues() {
		...
		while (iterator.hasNext()) {
			...
			if (values[x][y] < 1000
					&& eatAllIn4Steps(next_status, max_steps - 1)) {
				values[x][y] = 999;
				return;
			}
			boolean temp = true;
			...
			// 当4个顶角均为空且不在顶角和临近顶角的位置的空位的个数不多于1时，需要先对顶角的情况进行判断并重新设置各点的权值
				...
				if (judgeForTurn11(next_status, max_steps - 1) == 2) {
					if (super.is12(point) || super.is22(point))
						values[x][y] = 240;
					else
						values[x][y] = 260;
					temp = false;
				} else if (judgeForTurn11(next_status, max_steps - 1) == 1) {
					if (super.is12(point) || super.is22(point))
						values[x][y] = 200;
					else
						values[x][y] = 220;
				...
				if (super.is12(point)) {
					...
					if (super.toTurn11(next_status, self)
						...
						values[x][y] += 700;
							...
							&& last_status[chessboard_size - 2][chessboard_size - 2] == em)
							&& judgeForTurn11(next_status, max_steps - 1) == 2)
						values[x][y] += 300;
						...
						values[x][y] += 200;
							...
							&& (last_status[super.getOppo12(point).getX()][super
									...
									.getOppo12(point).getX()][super.getOppo12(
									point).getY()] == self
					...
					if (super.toTurn11(next_status, self)
						...
						if (super.canTurn13(next_status, em) < super.canTurn13(
								last_status, em)
								|| emToTurn13(next_status) < emToTurn13(last_status))
							values[x][y] += 20;
					...
					if (super.canTurn11(next_status, em) < super.canTurn11(
						...
						values[x][y] -= 30;
							...
							&& last_status[super.get24(point).getX()][super
									.get24(point).getY()] == em
							|| super.eat24(point, self))
						values[x][y] -= 20;
					else if (!super
							.illegal(next_status, super.get13(point), em)) {
						...
						if (super.canTurn13(last_status, self) == 0
							...
							.get33(point).getY()] == 0) {
						values[x][y] -= 2;
						if (judgeForTurn13(next_status, max_steps - 1) < 0)
							values[x][y] -= 3;
						else if (judgeForTurn13(next_status, max_steps - 1) == 2
								|| super.toTurn13(next_status, self) > super
										.toTurn13(last_status, self)
						...
						if (turn3Or4(next_status, super.getOppo13(point), self,
								false))
							values[x][y] += 2;
						else if (judgeForTurn13(next_status, max_steps - 1) < 0
								|| last_status[super.getOppo13(point).getX()][super
										.getOppo13(point).getY()] == em
								|| last_status[super.getOppo13(point).getX()][super
										.getOppo13(point).getY()] == 0
						...
						if (judgeForTurn13(next_status, max_steps - 1) == 2
								|| super.toTurn13(next_status, self) > super
										.toTurn13(last_status, self)
								|| super.canTurn13(next_status, em) < super
										.canTurn13(last_status, em))
					...
					if (super.canTurn11(next_status, em) < super.canTurn11(
						...
						if (super.canTurn13(last_status, self) == 0
							...
							&& !(last_status[super.getOppo13(point).getX()][super
									...
									.getOppo14(point).getX()][super.getOppo14(
									point).getY()] == em)) {
						values[x][y] += 2;
						...
						if (judgeForTurn13(next_status, max_steps - 1) == 2
								...
								|| super.canTurn13(next_status, em) < super
										.canTurn13(last_status, em))
							values[x][y] += 20;
					} else if (!super.illegal(next_status, super.get14(point),
							em)) {
						if (judgeForTurn13(next_status, max_steps - 1) < 0)
							values[x][y] -= 3;
						else if (next_status[super.get33(point).getX()][super
								.get33(point).getY()] == em
								&& next_status[super.getOppo34(point).getX()][super
								...
								&& next_status[super.get33(point).getX()][super
										.get33(point).getY()] == 0)
							values[x][y] -= 2;
						else if (turn3Or4(next_status, super.getOppo13(point),
								self, false))
							values[x][y] += 1;
						else if (last_status[super.getOppo13(point).getX()][super
								.getOppo13(point).getY()] == em
						...
						if (judgeForTurn13(next_status, max_steps - 1) < 0)
							values[x][y] -= 3;
						boolean t = true;
						if (!super.illegal(next_status, super.getOppo24(point),
								em)) {
							int[][] status2 = super.getNextStatus(next_status,
									super.getOppo24(point), em);
							if (super.illegal(status2, super.getOppo13(point),
									self)
									&& super.illegal(status2,
						...
						if (emToTurn13(next_status) > super.toTurn13(
							...
							&& (next_status[super.get33(point).getX()][super
									...
									.get33(point).getX()][super.get33(point)
									.getY()] == 0
									&& next_status[super.getOppo34(point)
											.getX()][super.getOppo34(point)
											.getY()] == em)) {
					...
					if (super.canTurn11(next_status, em) < super.canTurn11(
						...
						values[x][y] -= 20;
					else if (super.canTurn13(next_status, em) > super
							.canTurn13(last_status, em))
						values[x][y] -= 10;
					else if (judgeForTurn13(next_status, max_steps - 1) == 2
							|| super.toTurn13(next_status, self) > super
						...
						if (emToTurn13(next_status) > super.toTurn13(
							...
							values[x][y] -= 3;
						if (x == 2 && y == 2) {
							if (last_status[2][0] == self
									&& last_status[2][1] == em
									|| last_status[0][2] == self
									&& last_status[1][2] == em)
								values[x][y] += 4;
							...
							if (last_status[2][chessboard_size - 1] == self
									&& last_status[2][chessboard_size - 2] == em
									|| last_status[0][chessboard_size - 3] == self
									&& last_status[1][chessboard_size - 3] == em)
								values[x][y] += 4;
							...
							if (last_status[chessboard_size - 3][0] == self
									&& last_status[chessboard_size - 3][1] == em
									|| last_status[chessboard_size - 1][2] == self
									&& last_status[chessboard_size - 2][2] == em)
								values[x][y] += 4;
								...
								&& y == chessboard_size - 3) {
							if (last_status[chessboard_size - 3][chessboard_size - 1] == self
									&& last_status[chessboard_size - 3][chessboard_size - 2] == em
									|| last_status[chessboard_size - 1][chessboard_size - 3] == self
									&& last_status[chessboard_size - 2][chessboard_size - 3] == em)
								values[x][y] += 4;
					...
					if (super.canTurn33(next_status, em) > super.canTurn33(
							last_status, em))
						values[x][y] -= 1;
					int[] line = new int[chessboard_size];
					if (x == 3)
					...
					if (line[0] == self && line[1] != 0 && line[2] != 0
						...
						values[x][y] += 2;
						t = false;
					} else if (line[chessboard_size - 1] == 0
							&& line[chessboard_size - 2] != em
							&& line[chessboard_size - 3] == self
							&& line[chessboard_size - 4] == em && line[2] != em
							|| line[0] == 0 && line[1] != em && line[2] == self
							&& line[chessboard_size - 4] == 0
	 ...
	 * @param flag
	 *            递归参数，第1次调用时取flag=1，用于判断后面1-2步；第2次调用时取flag=0，用于判断后面3-4步
	 * @return +2：计算机在后面2步内能到达顶角 +1：计算机在后面4步内能到达顶角 0：双方在后面4步内都不能到达顶角
	 *         -1：对手在后面3步内能到达顶角 -2：对手在后面1步内能到达顶角
	 */
